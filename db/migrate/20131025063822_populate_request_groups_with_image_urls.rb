class PopulateRequestGroupsWithImageUrls < ActiveRecord::Migration
  def up
    RequestGroup.where(title: 'Windows application').first_or_create.update_attribute(:image_url, 'language_and_frameworks/windows.png')
    RequestGroup.where(title: 'Mac application').first_or_create.update_attribute(:image_url, 'language_and_frameworks/mac.png')
